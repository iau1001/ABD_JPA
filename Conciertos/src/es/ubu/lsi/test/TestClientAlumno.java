...
public class TestClientAlumno {
	...
	static void testService() throws Exception {
		...
		try {
			...
			desactivaEliminacionCorrecta(implService);
	...
	/**
	 * Comprueba las eliminaciones al desactivar el grupo.
	 * 
	 * @param implService implementación del servicio
	 * @throws Exception error en test
	 */
	private static void desactivaEliminacionCorrecta(Service implService) throws Exception {
		Connection con = null;
		Statement st = null;
		ResultSet rs = null;
		try {
			System.out.println("Comprobar datos eliminados tras desactivar grupo correcto");

			con = pool.getConnection();

			st = con.createStatement();
			rs = st.executeQuery("SELECT * FROM CONCIERTO JOIN COMPRA "
					+ "ON CONCIERTO.IDCONCIERTO = COMPRA.IDCONCIERTO "
					+ "where idgrupo = 1");
			
			if (rs.next()) {
				System.out.println("\tERROR no elimina los registros correctamente");
			}else {
				System.out.println("\tOK elimina los registros correctamente");
			}
			rs.close();
			con.commit();
		} catch (Exception ex) {
